...
begin
  ...
  begin
    ...
    begin
      ...
      pacienteNuevo.edad := copy(texto,0,pos('|',texto));
      delete(texto,0,pos('|',texto));

      pacienteNuevo.sexo := copy(texto,0,pos('|',texto));
      delete(texto,0,pos('|',texto));

      pacienteNuevo.codigoHistorial := copy(texto,0,pos('|',texto));
      delete(texto,0,pos('|',texto));

      pacienteNuevo.ingreso.dia := copy(texto,0,pos('|',texto));
      delete(texto,0,pos('|',texto));

      pacienteNuevo.ingreso.mes := copy(texto,0,pos('|',texto));
      delete(texto,0,pos('|',texto));

      pacienteNuevo.ingreso.anio := copy(texto,0,pos('|',texto));
      delete(texto,0,pos('|',texto));

      if (copy(texto,0,pos('|',texto) = 'true') then
        pacienteNuevo.ingreso.anio := True
      else
        pacienteNuevo.tieneSeguro := False;

      delete(texto,0,pos('|',texto));

      pacienteNuevo.totalFacturado := copy(texto,0,pos('|',texto));
